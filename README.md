...
- Bing Webmaster Tools verified through its site-specific DNS CNAME. The same sitemap reports **Success**, with **1 discovered URL**. Google import was canceled when it requested additional access across all verified Search Console sites.
- Google Business Profile phone and physical address match. Its existing **Cha In 茶穎** name was preserved. The Tuesday correction is published, making all seven days **12:00–21:30**. Google also published **https://chaincafe.my/** as the website; the Facebook social profile remains intact.

Preserve both search-engine verification DNS records. Monitor indexing/canonical selection and actual search data; avoid repeating indexing requests without new evidence. Indexing, inclusion in AI answers and ranking are not guaranteed. Actual dashboard results and first-party references are recorded in the [cutover log](docs/production-cutover-2026-09-15.md).
